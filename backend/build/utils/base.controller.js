...
function parseSortString(sort) {
    if (!sort)
        return {};
    return sort.split(",").reduce((acc, pair) => {
...
export class BaseController {
    app;
    constructor({ prefix, service, createSchema, updateSchema, responseSchema, swagger, extend, excludeRoutes = [], primaryKey = "id", // 🆕 مقدار پیش‌فرض id
     }) {
        const { tags } = swagger;
        const app = new Elysia({ prefix });
        const isEnabled = (route) => !excludeRoutes.includes(route);
        // 🧩 GET / (Get All)
        if (isEnabled("getAll")) {
            app.get("/", async ({ query }) => {
                const { page = 1, perPage = 20, sort, filter, include, paginate = false, } = query;
                let parsedFilter = {};
                let parsedInclude = {};
                if (filter) {
                    try {
        ...
        if (isEnabled("getOne")) {
            app.get(`/:${primaryKey}`, async ({ params, query }) => {
                let parsedInclude = {};
                if (query.include) {
                    try {
        ...
        if (isEnabled("update")) {
            app.put(`/:${primaryKey}`, async ({ params, body }) => {
                const keyValue = params[primaryKey];
        ...
        if (isEnabled("delete")) {
            app.delete(`/:${primaryKey}`, async ({ params, query }) => {
                const keyValue = params[primaryKey];
