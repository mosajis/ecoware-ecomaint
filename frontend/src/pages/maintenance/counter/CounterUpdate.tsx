...
export type CompCounterCurrentFormValues = z.infer<typeof schema>;
...
function CountersUpdate({ open, recordId, onClose, onSuccess }: Props) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CompCounterCurrentFormValues>({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch CompCounter", err);
      reset(defaultValues);
    } finally {
  ...
  const onSubmit = useCallback(
    async (values: CompCounterCurrentFormValues) => {
      if (!recordId) return;
      ...
      try {
        ...
        console.error("Update CompCounter failed", err);
      } finally {
        setSubmitting(false);
      }
    },
...
export default memo(CountersUpdate);
