describe("Location CRUD", () => {
  beforeEach(() => {
    ...
    cy.visit("/general/location");
  });

  it("should create, update, and then delete an location record", () => {
    const code = `CY-${Date.now()}`;
    const originalName = "Cypress Test Location";
    const updatedName = "Cypress Test Location (Updated)";

    // Create
    cy.get('[data-cy="add-button"]').click();
    cy.get('[data-cy="location-code-input"]').type(code);
    cy.get('[data-cy="location-name-input"]').type(originalName);
    ...
    cy.get('[data-cy="location-name-input"]').clear().type(updatedName);
    cy.get('[data-cy="form-submit"]').click();
  ...
  describe("Validation", () => {
    it("should show required errors when code and name are missing", () => {
      ...
      cy.get('[data-cy="location-code-error"]')
        .should("be.visible")
        .and("contain", "required");

      cy.get('[data-cy="location-name-error"]')
        .should("be.visible")
        .and("contain", "required");
    });
  });
  ...
  describe("Cancel", () => {
    it("should discard changes when the form dialog is cancelled", () => {
      ...
      cy.get('[data-cy="location-code-input"]').type(discardedCode);
